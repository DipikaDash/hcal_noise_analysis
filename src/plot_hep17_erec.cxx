...
int main()
{
  ...
  { 
    ...
    TH1D *h1_met[14][4][7]; // ieta iphi depth  
    ...
    for(int ieta=16; ieta<30; ieta++)
    {
      for(int iphi=63; iphi<67; iphi++)
      {
        for(int depth=1; depth<8; depth++)
        {
          ...
          if(infile->Get(Form("h1_met_erec_ieta%i_iphi%i_depth%i_nvtx%i", ieta,iphi,depth,invtx)))
          {
            h1_met[ieta-16][iphi-63][depth-1]
              = renormTH1D(static_cast<TH1D*>(infile->Get(Form("h1_met_erec_ieta%i_iphi%i_depth%i_nvtx%i", ieta,iphi,depth,invtx))));
            h1_met[ieta-16][iphi-63][depth-1]->Rebin(rebin);
            h1cosmetic(h1_met[ieta-16][iphi-63][depth-1], Form("ieta=%i,iphi=%i,depth=%i%s", ieta,iphi,depth,nvtxcut.Data()), kGreen+1,   1, 0, "E_{rechit} [GeV]"); 
            h1_met[ieta-16][iphi-63][depth-1]->SetMarkerSize(0);
            h1_met[ieta-16][iphi-63][depth-1]->GetXaxis()->SetRange(2,10000); 
          }
          else 
          { 
            h1_met[ieta-16][iphi-63][depth-1] = new TH1D(Form("h1_met_erec_ieta%i_iphi%i_depth%i_nvtx%i", ieta,iphi,depth,invtx),Form("h1_met_erec_ieta%i_iphi%i_depth%i_nvtx%i", ieta,iphi,depth,invtx),1,0,1);
          }
      ...
      for(int iphi=63; iphi<67; iphi++)
      {
        for(int depth=1; depth<8; depth++) 
        {
          ...
          { 
            ...
            h1_met_ratio[iphi-63][depth-1]->Divide(h1_zero[ieta-16][iphi-63][depth-1]);
...
//            h1_pion[ieta-16][iphi-63][depth-1]->Draw("e same");
            ...
            h1_jetht_ratio[iphi-63][depth-1]->GetYaxis()->SetTitle("Ratio");
            h1_jetht_ratio[iphi-63][depth-1]->GetYaxis()->SetTitleSize(0.15);
...
//            h1_pion_ratio[iphi-63][depth-1]->Draw("e same");
  ...
  //
  // compare high and low PU 
  //
  TH1D *h1_zero_nvtx1[14][4][7];  
  TH1D *h1_zero_nvtx2[14][4][7];  
    ...
    for(int iphi=63; iphi<67; iphi++)
    {
      for(int depth=1; depth<8; depth++)
      {
        ...
        { 
          ...
          h1_zero_nvtx1[ieta-16][iphi-63][depth-1]->GetXaxis()->SetRange(2,10000); 
        }
        else 
        { 
          h1_zero_nvtx1[ieta-16][iphi-63][depth-1] = new TH1D(Form("h1_zero_erec_ieta%i_iphi%i_depth%i_nvtx%i", ieta,iphi,depth,invtx),Form("h1_zero_erec_ieta%i_iphi%i_depth%i_nvtx%i", ieta,iphi,depth,invtx),1,0,1);
        } 
        invtx=2;
        if(infile->Get(Form("h1_zero_erec_ieta%i_iphi%i_depth%i_nvtx%i", ieta,iphi,depth,invtx))) 
        { 
          ...
          h1_zero_nvtx2[ieta-16][iphi-63][depth-1]->GetXaxis()->SetRange(2,10000); 
        }
        else 
        { 
          h1_zero_nvtx2[ieta-16][iphi-63][depth-1] = new TH1D(Form("h1_zero_erec_ieta%i_iphi%i_depth%i_nvtx%i", ieta,iphi,depth,invtx),Form("h1_zero_erec_ieta%i_iphi%i_depth%i_nvtx%i", ieta,iphi,depth,invtx),1,0,1);
        } 
      }
    }
  }
  ...
  l1->SetLineColor(kWhite);
  l1->SetShadowColor(kWhite);
  l1->AddEntry(h1_zero_nvtx1[5][0][0],   " ZeroBias, Nvtx<25",  "l");
  l1->AddEntry(h1_zero_nvtx2[5][0][0],   " ZeroBias, Nvtx>45",  "l");

  //
  for(int ieta=16; ieta<30; ieta++)
  {
    TH1D *h1_zero_ratio[4][7]; 
    TCanvas *c = new TCanvas("c","c",3200,1800);
    c->SetGrid(1,1); 
    c->Divide(7,4); 
    for(int iphi=63; iphi<67; iphi++)
    {
      for(int depth=1; depth<8; depth++) 
      {
        ...
        { 
          ...
          pad1->cd()->SetLogx(1);
          pad1->cd()->SetLogy(1); 
          h1_zero_nvtx2[ieta-16][iphi-63][depth-1]->GetXaxis()->SetLabelSize(0.0);
          h1_zero_nvtx2[ieta-16][iphi-63][depth-1]->GetYaxis()->SetLabelSize(0.06);
          h1_zero_nvtx2[ieta-16][iphi-63][depth-1]->Draw("e");
          h1_zero_nvtx1[ieta-16][iphi-63][depth-1]->Draw("e same");
          l1->Draw("same");
          // pad 2 
          c->cd((iphi-63)*7+depth);
          TPad *pad2 = new TPad("p_pull", "p_pull", 0.0, 0.0, 1.0, 0.3);
          ...
          pad2->cd();
          pad2->cd()->SetLogx(1);
          h1_zero_ratio[iphi-63][depth-1]->SetTitle("");
          h1_zero_ratio[iphi-63][depth-1]->SetMaximum(4);
          h1_zero_ratio[iphi-63][depth-1]->SetMinimum(0); 
          h1_zero_ratio[iphi-63][depth-1]->GetYaxis()->SetNdivisions(505);;
          h1_zero_ratio[iphi-63][depth-1]->GetYaxis()->SetTitle("Ratio");
          h1_zero_ratio[iphi-63][depth-1]->GetYaxis()->SetTitleSize(0.15);
          h1_zero_ratio[iphi-63][depth-1]->GetYaxis()->SetLabelSize(0.15);
          h1_zero_ratio[iphi-63][depth-1]->GetXaxis()->SetLabelSize(0.15);
          h1_zero_ratio[iphi-63][depth-1]->GetXaxis()->SetTitle("E_{rec} [GeV]");
          h1_zero_ratio[iphi-63][depth-1]->GetXaxis()->SetTitleSize(0.15);
          h1_zero_ratio[iphi-63][depth-1]->GetXaxis()->SetTitleOffset(1.2);
          h1_zero_ratio[iphi-63][depth-1]->Draw("e");
        }
      }
    }
    c->Print(Form("plots/h1_zero_erec_ieta%i.pdf", ieta));
    delete c;
  }
}
