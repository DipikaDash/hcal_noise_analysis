...
int main()
{
      ...
      for(int depth=1; depth<8; depth++)
      {
        ...
        { 
          h1_zero[ieta-16][iphi-63][depth-1]
           = renormTH1D(static_cast<TH1D*>(infile->Get(Form("h1_zero_erec_ieta%i_iphi%i_depth%i", ieta,iphi,depth))));
        ...
        if(infile->Get(Form("h1_jetht_erec_ieta%i_iphi%i_depth%i", ieta,iphi,depth)))
        {
          h1_jetht[ieta-16][iphi-63][depth-1]
           = renormTH1D(static_cast<TH1D*>(infile->Get(Form("h1_jetht_erec_ieta%i_iphi%i_depth%i", ieta,iphi,depth))));
        ...
        if(infile->Get(Form("h1_pion_erec_ieta%i_iphi%i_depth%i", ieta,iphi,depth)))
        {
          h1_pion[ieta-16][iphi-63][depth-1]
           = renormTH1D(static_cast<TH1D*>(infile->Get(Form("h1_pion_erec_ieta%i_iphi%i_depth%i", ieta,iphi,depth))));
          h1_pion[ieta-16][iphi-63][depth-1]->Rebin(1);
          h1cosmetic(h1_pion[ieta-16][iphi-63][depth-1], Form("ieta=%i,iphi=%i,depth=%i", ieta,iphi,depth), kBlue,   1, 0, "E_{rechit} [GeV]"); 
          h1_pion[ieta-16][iphi-63][depth-1]->SetMarkerSize(0);
          h1_pion[ieta-16][iphi-63][depth-1]->GetXaxis()->SetRange(2,10000); 
        }
        else 
        { 
          h1_pion[ieta-16][iphi-63][depth-1] = new TH1D(Form("h1_pion_erec_ieta%i_iphi%i_depth%i", ieta,iphi,depth),Form("h1_pion_erec_ieta%i_iphi%i_depth%i", ieta,iphi,depth),1,0,1);
        } 
      }
    }
  }
  ...
  for(int ieta=16; ieta<30; ieta++)
  {
    ...
    TCanvas *c = new TCanvas("c","c",3500,2400);
    c->Divide(7,4); 
    for(int iphi=63; iphi<67; iphi++)
    {
      for(int depth=1; depth<8; depth++) 
      {
        ...
        //c->cd((iphi-63)*7+depth)->SetLogy(1);
        //c->cd((iphi-63)*7+depth)->SetLogx(1);  
...
//          pad2->SetBottomMargin(0.4); 
          ...
          pad2->cd();
          pad2->cd()->SetLogx(1);
          h1_jetht_ratio[iphi-63][depth-1]->SetMaximum(2);
...
//          delete c;
    ...
    c->Print(Form("plots/h1_erec_ieta%i.pdf", ieta));
