#include "generate_hcal_tree.hpp"
...
set<Variable> GetVariables(const string &file_name){
  ...
  ifstream infile(("txt/hcal_tree_cfg/"+file_name).c_str());
  string line;
...
int main(int argc, char *argv[]){
  ...
  vector<string> file_names = Tokenize(execute("ls txt/hcal_tree_cfg/ 2> /dev/null"), "\n");
...
void WriteBaseHeader(const set<Variable> &all_vars,
  ...
  ofstream file("inc/hcal_tree.hpp");

  file << "// hcal_tree: base class to handle reduced tree ntuples\n";
  file << "// File generated with generate_hcal_tree.exe\n\n";

  file << "#ifndef H_hcal_tree\n";
  file << "#define H_hcal_tree\n\n";
  ...
  file << "class hcal_tree{\n";
  file << "public:\n";
  file << "  hcal_tree(); // Constructor to create tree\n";
  file << "  hcal_tree(const std::string &filename); // Constructor to read tree\n\n";
  ...
  file << "hcal_tree* NewTree(const std::type_info &type);\n\n";

  for(size_t i = 0; i < names.size(); ++i){
    file << "#include \"hcal_tree_" << names.at(i) << ".hpp\"\n";
  }
  file <<'\n';
...
void WriteBaseSource(const set<Variable> &all_vars,
                     const set<Variable> &com_vars,
                     const vector<string> &names){
  ofstream file("src/hcal_tree.cpp");

  file << "// hcal_tree: base class to handle reduce tree ntuples\n";
  file << "//File generated with generate_hcal_tree.exe\n\n";

  file << "#include \"hcal_tree.hpp\"\n\n";
  ...
  file << "bool hcal_tree::VectorLoader::loaded_ = false;\n\n";

  file << "hcal_tree::VectorLoader hcal_tree::vl_ = hcal_tree::VectorLoader();\n\n";

  file << "hcal_tree::VectorLoader::VectorLoader(){\n";
  file << "  if(!loaded_){\n";
  ...
  file << "const double hcal_tree::bad_val_ = -999.;\n\n";

  file << "hcal_tree::hcal_tree():\n";
  file << "  chain_(\"junk\", \"junk\"),\n";
  //file << "  tree_(\"tree\", \"tree\"),\n"; // FIXME
  file << "  tree_(\"hcalTupleTree/tree\", \"hcalTupleTree/tree\"),\n";
  file << "  entry_(0),\n";
  if(com_vars.size()){
  ...
  file << "hcal_tree::hcal_tree(const string &filename):\n";
  //file << "  chain_(\"tree\",\"tree\"),\n"; // FIXME
  file << "  chain_(\"hcalTupleTree/tree\",\"hcalTupleTree/tree\"),\n";
  ...
  file << "string hcal_tree::Type() const{\n";
  file << "  return \"\";\n";
  file << "}\n\n";
  ...
  file << "int hcal_tree::Add(const std::string &filename){\n";
  file << "  if(!read_only_){\n";
  ...
  file << "float hcal_tree::GetBranchValue(TString branch){\n";
  ...
  file << "bool hcal_tree::PassString(TString cut){\n";
  file << " TTreeFormula f(\"formula\",cut, &chain_);\n";
  file << " bool result = f.EvalInstance(0);\n";
  file << " return result;\n";
  file << "}\n\n";
  
  file << "long hcal_tree::GetEntries() const{\n";
  file << "  if(read_only_){\n";
  ...
  file << "void hcal_tree::GetEntry(const long entry){\n";
  file << "  if(!read_only_){\n";
  ...
  for(set<Variable>::const_iterator var = com_vars.begin(); var != com_vars.end(); ++var){
    file << var->type_ << " const & hcal_tree::" << var->name_ << "() const{\n";
    file << "  if(!read_only_){\n";
  ...
  for(set<Variable>::const_iterator var = com_vars.begin(); var != com_vars.end(); ++var){
    file << var->type_ << " & hcal_tree::" << var->name_ << "(){\n";
  ...
  for(set<Variable>::const_iterator var = all_vars.begin(); var != all_vars.end(); ++var){
    ...
    file << "  throw std::logic_error(\"" << var->name_
         << " does not exist in this hcal_tree version.\");\n";
    file << "}\n\n";
  }
  ...
  file << "hcal_tree* NewTree(const std::type_info &type){\n\n";
  file << "  if(type == typeid(hcal_tree)) return new hcal_tree;\n";
  for(size_t i = 0; i < names.size(); ++i){
    file << "  else if(type == typeid(hcal_tree_" << names.at(i) << ")) return static_cast<hcal_tree*>(new hcal_tree_" << names.at(i) << ");\n";
  }
  file << "  else return new hcal_tree;\n";
  file << "}\n\n";
...
void WriteSepHeader(const pair<string, set<Variable> > &sep_vars){
  ...
  ofstream file(("inc/hcal_tree_"+name+".hpp").c_str());

  file << "// hcal_tree_" << name << ": " << name << " version of hcal_tree to handle reduce tree ntuples\n";
  file << "// File generated with generate_hcal_tree.exe\n\n";

  file << "#ifndef H_hcal_tree_" << NAME << "\n";
  file << "#define H_hcal_tree_" << NAME << "\n\n";
  ...
  file << "class hcal_tree_" << name << " : public hcal_tree{\n";
  file << "public:\n";
  file << "  hcal_tree_" << name << "(); // Constructor to create tree\n";
  file << "  hcal_tree_" << name << "(const std::string &filename); // Constructor to read tree\n\n";
...
void WriteSepSource(const pair<string, set<Variable> > &sep_vars){
  ...
  string NAME = ToCaps(name);
  set<Variable> vars = sep_vars.second;
  ofstream file(("src/hcal_tree_"+name+".cpp").c_str());

  file << "// hcal_tree_" << name << ": " << name << " version of hcal_tree to handle reduce tree ntuples\n";
  file << "//File generated with generate_hcal_tree.exe\n\n";

  file << "#include \"hcal_tree.hpp\"\n\n";
  file << "#include \"hcal_tree_" << name << ".hpp\"\n\n";
  ...
  file << "hcal_tree_" << name << "::hcal_tree_" << name << "():\n";
  if(vars.size()){
  ...
  file << "hcal_tree_" << name << "::hcal_tree_" << name << "(const string &filename):\n";
  if(vars.size()){
    const set<Variable>::const_iterator vars_end_2 = --vars.end();
    file << "  hcal_tree(filename),\n";
    for(set<Variable>::const_iterator var = vars.begin();
        var != vars_end_2;
        ++var){
      file << "  " << var->name_ << "_(0),\n";
  ...
  file << "void hcal_tree_" << name << "::Fill(){\n";
  file << "  hcal_tree::Fill();\n";
  ...
  file << "string hcal_tree_" << name << "::Type() const{\n";
  file << "  return \"" << name << "\";\n";
  file << "}\n\n";
  ...
  file << "void hcal_tree_" << name << "::GetEntry(const long entry){\n";
  file << "  hcal_tree::GetEntry(entry);\n\n";
  ...
  for(set<Variable>::const_iterator var = vars.begin(); var != vars.end(); ++var){
    file << var->type_ << " const & hcal_tree_" << name << "::" << var->name_ << "() const{\n";
    file << "  if(!read_only_){\n";
    file << "    throw std::logic_error(\"Trying to write to const tree.\");\n";
    file << "  }\n";
    file << "  if(!c_" << var->name_ << "_ && b_" << var->name_ <<"_){\n";
  ...
  for(set<Variable>::const_iterator var = vars.begin(); var != vars.end(); ++var){
    file << var->type_ << " & hcal_tree_" << name << "::" << var->name_ << "(){\n";
    file << "  if(read_only_ && !c_" << var->name_ << "_ && b_" << var->name_ <<"_){\n";
    file << "    b_" << var->name_ << "_->GetEntry(entry_);\n";
    file << "    c_" << var->name_ << "_ = true;\n";
    file << "  }\n";
